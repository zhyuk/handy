...
const ScheduleChangeRequest = () => {
        ...
        {weeksData.map((week, wi) => (
          <div key={wi} className="grid grid-cols-7 mb-1">
            {week.map((d, di) => {
              ...
              return (
                <button key={di} onClick={() => { if (!d.isOutside) onSelect(key); }} className="pressable flex flex-col items-center py-1.5 w-full" style={{ minHeight: showSchedule ? '90px' : '56px' }} disabled={d.isOutside}>
  ...
  return (
    <div className="mx-auto min-h-screen max-w-lg bg-white">
      ...
      <div className="flex items-center gap-2 px-2 pt-4 pb-2" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      <Sheet open={clockInPickerOpen} onOpenChange={setClockInPickerOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl px-6 pb-8 pt-6 border-0 bg-white [&>button]:hidden max-h-[50vh]">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setClockInPickerOpen(false)} className="pressable"><X className="h-6 w-6 text-foreground" /></button>
          </div>
      ...
      <Sheet open={clockOutPickerOpen} onOpenChange={setClockOutPickerOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl px-6 pb-8 pt-6 border-0 bg-white [&>button]:hidden max-h-[50vh]">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setClockOutPickerOpen(false)} className="pressable"><X className="h-6 w-6 text-foreground" /></button>
          </div>
          <div className="overflow-y-auto max-h-[30vh] space-y-1">
            {TIME_OPTIONS.map((t) => (
      ...
      <Sheet open={reasonSheetOpen} onOpenChange={setReasonSheetOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl px-6 pt-6 border-0 bg-white [&>button]:hidden" style={{ paddingBottom: `${keyboardHeight + 32}px` }}>
      ...
      <ConfirmDialog
        ...
        title="일정 변경 요청하기"
        description={
          <span style={{ display: 'flex', flexDirection: 'column', gap: '4px', textAlign: 'center' }}>
            <span>아래와 같이 일정을 변경하시겠어요?<br />사장님이 확인 후 일정이 변경돼요</span>
            {selectedScheduleDate && selectedSchedule && (
              <span style={{ marginTop: '8px' }}>
                <span style={{ color: '#70737B' }}>선택한 일정 </span>
                <span style={{ color: '#19191B', fontWeight: 600 }}>{formatDateShort(selectedScheduleDate)} | {selectedSchedule.start}-{selectedSchedule.end}</span>
              </span>
            )}
            {changeDate && clockIn && clockOut && (
              <span>
                <span style={{ color: '#70737B' }}>변경할 일정 </span>
                <span style={{ color: '#4261FF', fontWeight: 600 }}>{formatDateShort(changeDate)} | {clockIn}-{clockOut}</span>
              </span>
            )}
          </span>
        }
        buttons={[
          { label: "취소", variant: "cancel", onClick: () => setConfirmDialogOpen(false) },
          { label: "확인", variant: "confirm", onClick: handleConfirm },
        ]}
      />
      </div>
    </div>
